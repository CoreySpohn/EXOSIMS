...
class OrbixUniverse(DulzPlavchanUniverseEarthsOnly):
    ...
    def create_orbix_planets(self, pInd, TK, SS, err=0.025, norb=500):
        ...
        Args:
            pInd (int): Index of the planet
            TK (TimeKeeping): TimeKeeping object
            err (float): 1-sigma error on the uncertain parameters
            norb (int): Number of planets to create
        """
        # Draw a and p based on the error
        _a = jnp.array(np.random.normal(self.a[pInd].to_value(u.AU), err, (norb,)))
        _p = jnp.array(np.random.normal(self.p[pInd], err, (norb,)))

        # _p = jnp.full(norb, self.p[pInd])
        # _a = jnp.full(norb, self.a[pInd].to_value(u.AU))
